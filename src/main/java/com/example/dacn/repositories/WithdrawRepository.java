...
public interface WithdrawRepository extends JpaRepository<Withdraw, Integer> {
    @Query(value = "select ifnull(sum(amount), 0) from withdraws inner join users on users.id = withdraws.user_id where username = ?1 and year(date) = ?2 and month(date) = ?3 and day(date) = ?4", nativeQuery = true)
